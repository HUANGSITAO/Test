...
## 2016.10.16 ##
晚上7点多来公司加班，7:40开干，很快将解决音频的代码移植到拼接的程序上，成功。但是音视频不同步。增加了同步代码的程序音视频更加的不同步。</br>
最新1.9是加了同步代码的，最新1.10是没加同步代码的，单1.9非常不同步。
## 2016.10.20 ##
主要解决音视频同步问题，但是还没有完全解决。</br>
将视频线程和音频线程编码的视频帧和音频帧分别存在两个队列中，根据dts来决定推流的顺序。</br>
这样做音频在视频后的2s左右，还有可能跑到音频的前边。</br>
## 2016.10.29 ##
